...
monitor = Monitor("https://blive.ericlamm.xyz", "nightbot")
...
def query(cid: int, response: Response):
...
def update(config: MC, response: Response):
    response.headers['Access-Control-Allow-Origin'] = '*'
...
def save(config: MC, response: Response):
    response.headers['Access-Control-Allow-Origin'] = '*'
    try:
        configs.append({
